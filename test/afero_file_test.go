...
func Test_AferoFile_happy(t *testing.T) {
	...
	comm.WriteFileTextP(fs, "/hello/world/test.txt", "hi")

	cred := &comm.CredentialsT{User: "u"}
	a1 := comm.NewAferoFileP(fs, "/hello/world/test.txt", cred, 3*time.Second)
	a2 := comm.NewAferoFileP(fs, "file:///hello/world/test.txt", cred, 3*time.Second)
