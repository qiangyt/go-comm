...
func Test_AferoFile_happy(t *testing.T) {
	...
	WriteFileTextP(fs, "/hello/world/test.txt", "hi")

	cred := &CredentialsT{User: "u"}
	a1 := NewAferoFileP(fs, "/hello/world/test.txt", cred, 3*time.Second)
	a2 := NewAferoFileP(fs, "file:///hello/world/test.txt", cred, 3*time.Second)
