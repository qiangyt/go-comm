...
func Test_Vars2Pair_happy(t *testing.T) {
	...
	a.Nil(Vars2Pair(nil))
	a.Nil(Vars2Pair(map[string]string{}))

	vars := map[string]string{
		"k1": "v1",
		"k2": "v2",
	}
	pairs := Vars2Pair(vars)
	a.Len(pairs, 2)
...
12`
	r := ParseCommandOutputP(text)
	a.Equal(text, r.Text)
	a.Equal(COMMAND_OUTPUT_KIND_JSON, r.Kind)
...
xyz`
	a.Panics(func() { ParseCommandOutputP(text) }, "json: xyz")
}
...
Key=Value`
	r := ParseCommandOutputP(text)
	a.Equal(text, r.Text)
	a.Equal(COMMAND_OUTPUT_KIND_VARS, r.Kind)
	a.Len(r.Vars, 1)
...
func Test_ParseCommandOutput_text(t *testing.T) {
	...
	r := ParseCommandOutputP(text)
	a.Equal(text, r.Text)
	a.Equal(COMMAND_OUTPUT_KIND_TEXT, r.Kind)
	a.Len(r.Vars, 0)
	a.Nil(r.Json)
}
