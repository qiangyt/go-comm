...
func RequiredFloat(hint string, key string, m map[string]any) (float64, error) {
	v, has := m[key]
	if !has {
		return 0, q18n.LocalizeError("error.required", map[string]any{
			"Hint": hint,
			"Key":  key,
		})
	}
...
func Float(hint string, v any) (float64, error) {
	...
	if !ok {
		...
		return 0, q18n.LocalizeError("error.type.float", map[string]any{
			"Hint":  hint,
...
func FloatArray(hint string, v any) ([]float64, error) {
	...
	if !ok {
		...
		return nil, q18n.LocalizeError("error.type.float_array", map[string]any{
			"Hint":  hint,
			"Type":  reflect.TypeOf(v),
			"Value": v,
		})
...
func FloatMap(hint string, v any) (map[string]float64, error) {
	...
	if !ok {
		...
		return nil, q18n.LocalizeError("error.type.float_map", map[string]any{
			"Hint":  hint,
			"Type":  reflect.TypeOf(v),
			"Value": v,
		})
