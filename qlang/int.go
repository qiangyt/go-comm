...
func Int(hint string, v any) (int, error) {
	...
	if !ok {
		...
		return 0, q18n.LocalizeError("error.type.int", map[string]any{
			"Hint":  hint,
...
func IntArray(hint string, v any) ([]int, error) {
	...
	if !ok {
		...
		return nil, q18n.LocalizeError("error.type.int_array", map[string]any{
			"Hint":  hint,
			"Type":  reflect.TypeOf(v),
			"Value": v,
		})
...
func IntMap(hint string, v any) (map[string]int, error) {
	...
	if !ok {
		...
		return nil, q18n.LocalizeError("error.type.int_map", map[string]any{
			"Hint":  hint,
			"Type":  reflect.TypeOf(v),
			"Value": v,
		})
