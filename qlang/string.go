...
func RequiredString(hint string, key string, m map[string]any) (string, error) {
	v, has := m[key]
	if !has {
		return "", q18n.LocalizeError("error.required", map[string]any{
			"Hint": hint,
			"Key":  key,
		})
	}
...
func String(hint string, v any) (string, error) {
	r, ok := v.(string)
	if !ok {
		return "", q18n.LocalizeError("error.type.string", map[string]any{
			"Hint":  hint,
...
func StringArrayValue(hint string, key string, m map[string]any) ([]string, error) {
	v, has := m[key]
	if !has {
		return nil, q18n.LocalizeError("error.required", map[string]any{
			"Hint": hint,
			"Key":  key,
		})
	}
...
func StringArray(hint string, v any) ([]string, error) {
	...
	if !ok {
		...
		return nil, q18n.LocalizeError("error.type.string_array", map[string]any{
			"Hint":  hint,
			"Type":  reflect.TypeOf(v),
			"Value": v,
		})
...
func StringMap(hint string, v any) (map[string]string, error) {
	...
	if !ok {
		...
		return nil, q18n.LocalizeError("error.type.string_map", map[string]any{
			"Hint":  hint,
			"Type":  reflect.TypeOf(v),
			"Value": v,
		})
