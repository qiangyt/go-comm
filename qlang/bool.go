...
func RequiredBool(hint string, key string, m map[string]any) (bool, error) {
	v, has := m[key]
	if !has {
		return false, q18n.LocalizeError("error.required", map[string]any{
			"Hint": hint,
			"Key":  key,
		})
	}
...
func Bool(hint string, v any) (bool, error) {
	...
	if !ok {
		...
		return false, q18n.LocalizeError("error.type.bool", map[string]any{
			"Hint":  hint,
...
func BoolArray(hint string, v any) ([]bool, error) {
	...
	if !ok {
		...
		return nil, q18n.LocalizeError("error.type.bool_array", map[string]any{
			"Hint":  hint,
			"Type":  reflect.TypeOf(v),
			"Value": v,
		})
...
func BoolMap(hint string, v any) (map[string]bool, error) {
	...
	if !ok {
		...
		return nil, q18n.LocalizeError("error.type.bool_map", map[string]any{
			"Hint":  hint,
			"Type":  reflect.TypeOf(v),
			"Value": v,
		})
